...
int plotCrystalsEdep(const char *fileName) {
  ...
  for (Int_t i = 0; i < nHists; i++) {
    TString edepHistName = TString::Format("matrixEdep%d", i);
    // TString edepHistSafeName = StringUtils::getSafeName(edepHistName.Data());
  ...
  for (Int_t i = 0; i < nHists; i++) {
    ...
    Double_t percentDepositedEnergy = totalEdep[i] / totalGPSEnergy * 100.;
  ...
  // Close File
  // This also deletes histograms created after opening the file
  file->Close();
  delete file;
...
// ---------------------------------------------------------------
// Compose histograms with optical output from unit volume centers
// ---------------------------------------------------------------
...
// ---------------------------------
// Fitting of the resolution points
// ---------------------------------
...
TVector3 getEnergyResolution(const char *fileName) {
  ...
  // Obtain ranges of energy deposited in all events (written as array in the tree)
  Double_t *edep = new Double_t[nCrystals];
  tree->SetBranchAddress("edep", edep);

  Double_t branchMin = DBL_MAX;
  Double_t branchMax = DBL_MIN;
  for (int i = 0; i < (int) tree->GetEntries(); i++) {
    ...
    branchMin = TMath::Min(branchMin, totalEdep);
    branchMax = TMath::Max(branchMax, totalEdep);
  }

  branchMax = branchMax * 1.25;

  // Ensure histogram bin width is fixed
  // This is important for plotting in gnuplot multiple edep results
  Double_t binWidth = 15; // MeV/bin
  Int_t nBins = (branchMax - branchMin) / binWidth + 1;
  branchMax = branchMin + binWidth * nBins;

  // Instantiate histogram for total energy deposition
  TH1D edepHist("edepAll", "", nBins, branchMin, branchMax);
  TString yAxisTitle = TString::Format("Counts / %.0f MeV", binWidth);
  edepHist.GetYaxis()->SetTitle(yAxisTitle.Data());
  edepHist.GetXaxis()->SetTitle("Total deposited energy, MeV");

  // Fill total energy deposition histogram from the tree
  for (Int_t i = 0; i < (Int_t) tree->GetEntries(); i++) {
    // Get tree entry
    tree->GetEntry(i);
  ...
  CanvasHelper::getInstance()->addCanvas(canvas);
  PathComponents pc = FileUtils::parseFilePath(fileName);
  TString canvasName = TString::Format("%s-eres", pc.name.Data());
  canvas->SetName(canvasName.Data());
  CanvasHelper::saveCanvas(canvas, kFormatPng);
  FileUtils::exportCanvasToGnuplot(canvas);
...
TVector3 getOpticalResolution(const char *fileName) {
  ...
  // Ensure histogram bin width is fixed
  // This is important for plotting in multiple PE hists in gnuplot
  Double_t binWidth = 450; // PE/bin
  Int_t nBins = (branchMax - branchMin) / binWidth + 1;
  branchMax = branchMin + binWidth * nBins;

  // Instantiate histogram for total energy deposition
  TString histName = StringUtils::getSafeName("peHist");
  TH1D *hist = new TH1D(histName.Data(), "PE Distribution in Sensitive Detector", nBins, branchMin, branchMax);
  TString yAxisTitle = TString::Format("Counts / %.0f MeV", binWidth);
  hist->GetYaxis()->SetTitle(yAxisTitle.Data());
  hist->GetXaxis()->SetTitle("Number of Detected PE");
  ...
  CanvasHelper::getInstance()->addCanvas(canvas);
  PathComponents pc = FileUtils::parseFilePath(fileName);
  TString canvasName = TString::Format("%s-optres", pc.name.Data());
  canvas->SetName(canvasName.Data());
  CanvasHelper::saveCanvas(canvas, kFormatPng);
  FileUtils::exportCanvasToGnuplot(canvas);
...
// Get number of events, GPS particle energy and particle name
  ...
  TString title = TString::Format(
      "%s Resolution Series for %s  Crystal Assembly;Incident particle energy, GeV;%s resolution, %%;",
      isOptical ? "Optical" : "Energy", crystalMaterial, isOptical ? "Optical" : "Energy");
