...
void DetectorConstruction::DefineMaterials() {
    ...
    // PMT vacuum
    fPMTVacuumMater = Materials::getInstance()->getMaterial(
...
void DetectorConstruction::ConstructVolumes() {
    ...
    // Visual attributes:
    auto crystalVisAttr = new G4VisAttributes(G4Color(0.1, 1.0, 0.9, 1.));
    crystalVisAttr->SetLineWidth(1);
    crystalVisAttr->SetForceSolid(true);

    auto pmtVisAttr = new G4VisAttributes(G4Color(0.3, 0.5, 0.9, 0.9));
    ...
    G4double single_X = std::max(fCrystal_X + 2 * fWrapThickness + fGap,
                                 detectorHeight); // max(crystalWidth, detectorWidth)
    G4double single_Y = std::max(fCrystal_Y + 2 * fWrapThickness + fGap,
                                 detectorHeight); // max(crystalHeight, detectorHeight)
    ...
    G4LogicalVolume *lMother = new G4LogicalVolume(sMother  /* solid */, fWorldMater /* material */,
                                                   "mother_logical" /* name */);
    ...
    G4Box *singleS = new G4Box("module_assembly", 0.5 * single_X, 0.5 * single_Y, 0.5 * single_Z);
    G4LogicalVolume *singleLV = new G4LogicalVolume(singleS,           // solid
                                                    fWorldMater,       // material
                                                    "module_assembly_logical"); // name
    ...
    G4Box *crystalSolid = new G4Box("crystal", 0.5 * fCrystal_X, 0.5 * fCrystal_Y, 0.5 * fCrystal_Z);
    fLogicCrystal = new G4LogicalVolume(crystalSolid,   // its solid
                                        fDetectorMater, // its material
                                        "crystal_logical"); // its name

    fLogicCrystal->SetVisAttributes(crystalVisAttr);
    ...
    if (fWrapMater) {
        ...
        G4SubtractionSolid *wrapSolid = new G4SubtractionSolid("wrap_solid", wrapASolid, wrapBSolid, wrapRot,
                                                               wrapTrans);
        ...
        if (isHybrid()) {
            G4VPhysicalVolume *wrapOutPV = new G4PVPlacement(0, G4ThreeVector(0, 0, wrap_pos_Z), fLogicWrap,
                                                             "wrapOutside_physical", singleOutLV, false, 0,
                                                             checkOverlaps);
    ...
    if (AbsPMT *pmt = dynamic_cast<AbsPMT *>(detector)) {
        ...
        if (fGreaseThickness > 0) {
            ...
            new G4PVPlacement(0, G4ThreeVector(0, 0, greasePosZ), fLogicGrease, "grease_physical", singleLV, false, 0,
                              checkOverlaps);
            if (isHybrid()) {
                new G4PVPlacement(0, G4ThreeVector(0, 0, greasePosZ), fLogicGrease, "greaseOutside_physical",
                                  singleOutLV,
        ...
        new G4PVPlacement(0, G4ThreeVector(0, 0, PMT_pos_Z), fLogicPMTCase, "pmt_physical", singleLV, false, 0,
                          checkOverlaps);
        if (isHybrid()) {
            new G4PVPlacement(0, G4ThreeVector(0, 0, PMT_pos_Z), fLogicPMTCase, "pmtOutside_physical", singleOutLV,
                              false, 0,
                              checkOverlaps);
        }
        fLogicPMTCase->SetVisAttributes(pmtVisAttr);
        ...
        new G4PVPlacement(0, G4ThreeVector(0, 0, pmtCathodePosZ), fLogicPMTCathode, "pmtCathode_physical",
                          fLogicPMTCase,
                          false, 0, checkOverlaps);
        ...
        G4Box *sPMTcoverOuter = new G4Box("pmtCoverOut_solid", 0.5 * (single_X), 0.5 * (single_Y),
                                          0.5 * pmt->getLength());
        ...
        G4SubtractionSolid *sPMTcover = new G4SubtractionSolid("pmtCover_solid", sPMTcoverOuter, sPMTcoverInner,
                                                               coverRot,
                                                               coverTrans);
        ...
        new G4PVPlacement(0, G4ThreeVector(0, 0, PMT_pos_Z), fLogicPMTshield, "pmtCover_physical", singleLV, false, 0,
                          checkOverlaps);
        if (isHybrid()) {
            new G4PVPlacement(0, G4ThreeVector(0, 0, PMT_pos_Z), fLogicPMTshield, "pmtShieldOutside_physical",
                              singleOutLV,
                              false, 0, checkOverlaps);
        }
        ...
        // GEOMETRY: World volume -> Mother volume -> Temperature control box -> Single volume -> Optical grease
        if (fGreaseThickness > 0) {
            G4Box *greaseSolid = new G4Box("grease_solid", mppc->getSize() / 2, mppc->getSize() / 2,
                                           fGreaseThickness / 2);
        ...
        G4SubtractionSolid *mppcReflectorSolid = new G4SubtractionSolid("mppcReflector_solid", mppcReflectorOuter,
                                                                        mppcReflectorInner, reflectorRot,
                                                                        reflectorTrans);
        ...
        new G4PVPlacement(0, G4ThreeVector(0, 0, mppcShieldPosZ), fLogicMPPCShield, "mppcCase_physical", singleLV,
                          false,
                          0, checkOverlaps);
        if (isHybrid()) {
        ...
        new G4PVPlacement(0, G4ThreeVector(0, 0, mppcWindowPosZ), fLogicMPPCWindow, "mppcWindow_physical", singleLV,
                          false,
                          0, checkOverlaps);
        if (isHybrid()) {
            new G4PVPlacement(0, G4ThreeVector(0, 0, mppcWindowPosZ), fLogicMPPCWindow, "mppcWindowOut_physical",
                              singleLV,
                              false, 0, checkOverlaps);
        }
        ...
        new G4PVPlacement(0, G4ThreeVector(0, 0, mppcPosZ), fLogicMPPC, "mppc_physical", singleLV, false, 0,
                          checkOverlaps);
        if (isHybrid()) {
            new G4PVPlacement(0, G4ThreeVector(0, 0, mppcPosZ), fLogicMPPC, "mppcOutside_physical", singleOutLV, false,
                              0,
                              checkOverlaps);
        }
    }
