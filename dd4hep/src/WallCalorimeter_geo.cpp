...
//  // GLASS
    ...
    if (towersInRow % 2) {
        ...
        //      [ ][ ][ ][ ][ ]
        //       ^     |
         towersInHalfRow = std::floor(towersInRow / 2.0);
...
//
    ...
    for (int rowIndex = 0; rowIndex < towersInRow; rowIndex++) {
        for (int colIndex = 0; colIndex < towersInRow; colIndex++) {
            ...
            fmt::print("{:>15} {:>15} {:>15.2f} {:>15.2f}\n" , rowIndex, colIndex, glass_x, glass_y);
            // glass module
            auto placement = ecal_vol.placeVolume(glass_module, Position(glass_x, glass_y, Glass_z0 + Glass_offset));
            placement.addPhysVolID("row", rowIndex);
            placement.addPhysVolID("col", colIndex);
...
//@}
DECLARE_DETELEMENT(epic_WallCalorimeter, create_detector)
