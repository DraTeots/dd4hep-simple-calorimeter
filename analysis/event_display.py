...
class EcalGeomInfo:
    ...
    def coord_to_index(self, x, y):
        ...
        index_x = coord_to_index(x, self.min_x, self.module_size_x*10)          # *10 to convert cm to mm
        index_y = coord_to_index(y, self.min_y, self.module_size_y*10)          # *10 to convert cm to mm
    ...
    def read_events_from_file(self, data_file_name, entry_start=0, entry_stop=1):
        ...
        for i in range(entry_start, entry_stop):
            ...
            energies = ak.flatten(tree['WallECalHits/WallECalHits.energy']
                                  .array(entry_start=i, entry_stop=i+1)).to_numpy()
            hits_x = ak.flatten(tree['WallECalHits/WallECalHits.position.x']
                                .array(entry_start=i, entry_stop=i+1)).to_numpy()
            hits_y = ak.flatten(tree['WallECalHits/WallECalHits.position.y']
                                .array(entry_start=i, entry_stop=i+1)).to_numpy()
            hits_z = ak.flatten(tree['WallECalHits/WallECalHits.position.z']
                                .array(entry_start=i, entry_stop=i + 1)).to_numpy()
...
def gdml_read_ecal_info(file_name):
    ...
    msize_x, msize_y, msize_z, unit = gdml_read_module_geometry('glass_box', geometry_xml)

    # Read module positions
    mpositions = gdml_read_module_positions('glass_module', geometry_xml)
...
def table_display(event_data, fig=None, ax=None, cal_size=12, cell_size=1, value_is_arr=True):
    """
    ...
    for y_iter in range(cal_size):
        for x_iter in range(cal_size):
            x = centers[x_iter]
            y = centers[y_iter]
            weight = event_data[x_iter][y_iter][0] if value_is_arr else event_data[x_iter][y_iter]
            #print(x,y,weight)
            patch = patches.Rectangle((x-dx, y-dy), size_x, size_y, edgecolor='black', facecolor=cmap(norm(weight)))
            module_rects.append(patch)
